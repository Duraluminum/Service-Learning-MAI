...
async def send_daily_reminders():
    while True:
        try:
            ...
            target_dates = [
                (now_for_deadline + timedelta(days=1)).isoformat(),
                (now_for_deadline + timedelta(days=3)).isoformat()
            ]
            ...
            async with async_session() as session:
                tasks = await session.scalars(
                    ...
                    .where(Task.deadline.in_(target_dates))
                )
                task_list = tasks.all()
                ...
                for user_id, user_tasks in tasks_by_user.items():
                    ...
                    if task_lines:
                        ...
                        try:
                            ...
                            print(f'Не удалось отправить сообщение пользователю {user.tg_id}')
                        except TelegramForbiddenError:
                            print(f'Пользователь {user.tg_id} заблокировал бота')
